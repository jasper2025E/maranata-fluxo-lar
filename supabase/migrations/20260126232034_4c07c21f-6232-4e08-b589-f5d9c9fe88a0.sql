CREATE OR REPLACE FUNCTION public.gerar_faturas_aluno(
  ...
  p_valor numeric, 
  p_data_inicio date,
  p_quantidade_meses integer DEFAULT 12
)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Extrai o dia de vencimento da data de início
  v_dia_vencimento := EXTRACT(DAY FROM p_data_inicio);

  FOR i IN 0..(p_quantidade_meses - 1) LOOP
    -- Calcula o mês/ano de vencimento
    ...
    -- Ajusta a data de vencimento para o dia correto
    v_data_vencimento := DATE_TRUNC('month', v_data_vencimento) + ((v_dia_vencimento - 1) || ' days')::INTERVAL;
